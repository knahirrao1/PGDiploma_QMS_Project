...
const Quiz = (props) => {
  ...
  useEffect(()=>{
    const fetchData = async()=>{
      await axios.get(`${server}/quizhub/questions/quizzes/${props.quizId}`)
      .then((res)=>{
  ...
  return (
    ...
    // </div>
    <div className="container" style={{ backgroundColor: '#ffffcc', borderRadius: '10px', padding: '20px', marginTop: '20px', boxShadow: '0px 0px 10px 0px rgba(0,0,0,0.1)' }}>
          ...
          <div className="question" key={question.question_id}>
            ...
            <div className="form-check">
              <input className="form-check-input custom-radio" type="radio" name={`question${question.question_id}`}
              id={`optionA${question.question_id}`}
              value="A"
              onChange={(event) => handleOptionChange(event, question.question_id)} />
